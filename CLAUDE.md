...
- **Temporal evaluation (Goal 6/7).** `main.py` now saves per-question predictions to `outputs/<dataset>/predictions_tw_{on,off}.json` (named by `temporal_weighting`). `reproduce/eval_temporal.py` reads those offline (no GPU) and reports per-category EM/F1, gold-hit (normalized containment), and **stale-rate** (wrong-version answers; defined for `latest_wins`/`conflict`/`time_specific`), comparing on/off side by side. Workflow: run `main.py` twice (`--temporal_weighting false`/`true`) → `python reproduce/eval_temporal.py`.
