...
- **D1 — per-fact time anchor (opt-in, default off).** `--fact_time_anchor true` runs a post-OpenIE pass (`HippoRAG._extract_fact_times`) where the LLM assigns a timestamp to each extracted triple (adapted from DyG-RAG's temporal-parsing protocol: formats `YYYY-MM-DD|YYYY-MM|YYYY|static`, relative dates resolved from the passage). `_parse_time_to_int` converts these to sortable ints (`YYYYMMDD`, `static`→0), and `add_fact_edges` puts the per-fact time on the fact edge instead of the doc-level timestamp when a real (>0) one exists. One LLM call per chunk, cached in `fact_times.json` under the working dir. `index()` also builds **`fact_time_by_text`** (`{str(tuple(triple)): time}`, persisted to `fact_time_by_text.json`) so D2/D3 read each fact's OWN time by triple text rather than the entity-pair edge time — this avoids the edge-level latest-wins collapse (two facts sharing a subject/object pair, e.g. joined@2015 vs resigned@2022, keep distinct times for timeline/scoping). The edge attribute still collapses to latest (used by recency-PPR, which is edge-level by design). Gated so the default path and retrieval-only sessions are unchanged; the graph lives in a separate `_ft` working dir (set in `main.py`) so the baseline graph is never overwritten. Motivation: real datasets (timeqa/complextr) carry no per-passage timestamp (=0), so without D1 every downstream temporal mechanism (D2 timeline, recency-PPR) is inert. **Status: code complete + syntax-checked; recency-PPR over D1 anchors gave ≈ −1pp on complextr (within noise) — recency is the wrong consumer; D2 is the intended one.**
- **D2 — timeline + Time-CoT (opt-in, default off).** `--time_cot true` makes `retrieve()` attach the reranked `top_k_facts` with their **per-fact timestamps** (`_fact_time`, keyed by the triple text via `fact_time_by_text` — NOT the entity-pair edge time, so `(X,joined,Y)@2015` and `(X,resigned,Y)@2022` keep their own times instead of collapsing to latest) onto `QuerySolution.facts`; `qa()` then renders them chronologically via `_build_timeline_block` ("Event N [time]: s p o", timeless facts as Background) plus a temporal-reasoning instruction (adapted from DyG-RAG `dynamic_QA`/Time-CoT), prepended to the QA prompt before the passages. **QA-time only — does NOT touch the graph, so it reuses the working dir** (no new suffix); predictions are saved as `predictions_tw_{on,off}_cot_on.json` and `eval_dyg.py` matches them. Inert unless edges carry real timestamps → pair with `--fact_time_anchor` on real datasets. **Status: code complete + syntax-checked.** On complextr (clean cache-copied run, gpt-4.1-nano temp=0) D2 gave a small, mixed effect: +1.8pp EM but ≈neutral Acc/Recall/F1 — modest, dataset-dependent. Conceptual limit: D2 only helps the LLM reason over already-retrieved evidence — it does NOT make *retrieval* time-aware (wrong-time passages stay wrong); that is D3.
- **D3 — time-scoped retrieval (opt-in, default off).** `--time_scoped true` multiplies fact scores in `get_fact_scores` by a Gaussian temporal-proximity weight: `_parse_query_year` reads the year the question asks about (last 4-digit year in the query), `_apply_time_scope` down-weights facts whose edge timestamp is far from that year by an **exponential decay PER DAY** (mirrors DyG-RAG): `exp(-time_scope_decay_rate * |Δdays|)`, `time_scope_decay_rate` default 0.01 (sharp on year-granular data: 1yr≈365d→0.026; tune down if it over-suppresses). Fact times are converted to day-ordinals (`_ymd_to_days`) once in `prepare_retrieval_objects` (`self.fact_day_ordinals`). Facts with no real time (≤0) and queries with no parseable year stay neutral (×1.0). Per-fact times precomputed once in `prepare_retrieval_objects` into `self.fact_times` (aligned to `fact_node_keys`, via **`_fact_time`** = per-fact time keyed by triple text in `fact_time_by_text`, NOT the collapsed edge time), only when `time_scoped` on. **UNLIKE recency (`temporal_weighting`=newest-wins), D3 scopes to the *asked* time** — the mechanism the `time_specific` failures point at. Retrieval-time only (reuses graph/working dir); predictions tagged `..._ts_on.json`. Pair with `fact_time_anchor` for real timestamps. **Status: code complete + syntax-checked + unit-tested (proximity math); NOT yet validated on a real LLM run** — best tested on timeqa (`time_specific`), since complextr questions may lack parseable years.
- **Information filtering (opt-in, default off).** `--info_filter true` drops low-value triples (`_is_informative_triple`) **before** graph construction, in `index()` right after `chunk_triples` is built (so entity nodes, facts, and D1 all see the filtered set). Faithful to DyG-RAG's two-part rule: (a) HARD "explicit subject, no pronouns" — drop if subject/object is a bare pronoun/stopword (`_LOW_INFO_TOKENS`), empty, or <2 chars; (b) SOFT info-value score (keep if ≥1) over 4 DyG criteria via cheap proxies — specific actor (both endpoints substantive), action/state-change verb (`_STATE_VERBS` in predicate), quantitative (a digit), temporal anchoring (a 4-digit year). No LLM. **Honest note: on HippoRAG's already-structured triples the actor proxy is almost always satisfied → the soft score rarely fires beyond the hard pronoun rule** (the structured extraction already did DyG's sentence-level filtering). So info_filter ≈ the pronoun rule in practice. Changes the graph → separate working dir (`main.py` appends `_if`, composes with `_ft`). The `main.py` OpenIE-reuse convenience now covers any derived dir (`_ft` and/or `_if`): it strips those suffixes to find the base (plain) OpenIE cache and copies it in, so the derived graph is built from the same triples as the baseline. **Status: code complete + syntax-checked + unit-tested; not yet validated — benefit uncertain (prunes edges → could help noise or hurt recall), measure on timeqa.**
- **A — temporal-proximity edges (opt-in, default off). Faithful to DyG-RAG's `EventRelationshipConfig` (graphrag/_op.py).** `--temporal_edges true` runs `add_temporal_edges` after `add_fact_edges` in `index()`. Each triple is a pseudo-event (entities {subj,obj} + D1 time). For each fact, find co-entity facts and score each by DyG's formula: `entity_weight=min(1, entity_factor·#common)` (`entity_factor`=0.2), `time_weight=time_factor·exp(-decay_rate·|Δdays|)` (`decay_rate`=0.01 **per day**, `time_factor`=1.0), `combined=entity_ratio·entity_weight+time_ratio·time_weight` (0.6/0.4); keep top `max_links` (=3); **no hard time window** (soft decay + top-N, as in DyG). The combined score (×`temporal_edge_weight`) is added into `node_to_node_stats`. **DIVERGENCE from DyG:** DyG links event NODES; HippoRAG has no event nodes, so we bridge the non-shared ENTITIES instead (entity-graph adaptation). Candidates per fact capped at 200. Requires real per-fact timestamps → **pair with `--fact_time_anchor`** (no-op without). Changes the graph → separate `_te` working dir (composes `_ft`/`_if`; `main.py` OpenIE-reuse strips all of `_te`/`_if`/`_ft`). Tunable flags: `--temporal_edge_decay_rate`/`--temporal_edge_max_links`/`--temporal_edge_weight` (ratios/factors are config fields at DyG defaults). All numeric defaults = DyG's published code values (the paper itself does NOT publish them). **Status: code complete + syntax-checked + unit-tested; NOT yet validated.** Most-promising un-adopted DyG idea that fits the PPR substrate; targets the time-blind multi-hop gap (−12pp to DyG on ComplexTR).
...
- **Time-scoped retrieval — PARTIALLY DONE (D3, see Implemented).** `--time_scoped` now biases fact scores toward the year parsed from the question (Gaussian proximity). Still unconsumed: the corpus `valid_from`/`valid_to` validity intervals and the dataset's explicit `question_time` field (D3 re-parses the year from the query text instead). Interval/validity-aware scoping (IA-RAG style) remains unbuilt.
