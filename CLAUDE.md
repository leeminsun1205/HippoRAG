...
- **Temporal evaluation metrics.** The `temporal_mvp` dataset exists (see below), but scoring is still only the generic EM/F1/Recall in `main.py`; per-category / stale-rate / conflict-accuracy metrics are not built yet.
...
In short: recency now optionally affects retrieval (Goal 5, behind `temporal_weighting`); conflict/supersede, provenance-weighting, time-scoped retrieval, and temporal metrics are still unbuilt. Tasks touching those mean building new logic, not editing existing logic.
...
Question entries add `category`, `question_time`, `gold_timestamp`, `gold_claim_id` on top of the standard `question`/`answer`/`paragraphs` (so existing eval works). Because only recency-PPR exists so far, `time_specific` is expected to score low (answered with the newest version) — that's the motivation for time-scoped retrieval, not a bug.
