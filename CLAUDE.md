...
- **D3 — time-scoped retrieval (opt-in, default off).** `--time_scoped true` multiplies fact scores in `get_fact_scores` by a Gaussian temporal-proximity weight: `_parse_query_year` reads the year the question asks about (last 4-digit year in the query), `_apply_time_scope` down-weights facts whose edge timestamp is far from that year (`exp(-(Δyears/tau)^2)`, `tau`=`time_scope_tau`, default 3.0). Facts with no real time (≤0) and queries with no parseable year stay neutral (×1.0). Per-fact times precomputed once in `prepare_retrieval_objects` into `self.fact_times` (aligned to `fact_node_keys`, read from graph edges via `_fact_edge_time`), only when `time_scoped` on. **UNLIKE recency (`temporal_weighting`=newest-wins), D3 scopes to the *asked* time** — the mechanism the `time_specific` failures point at. Retrieval-time only (reuses graph/working dir); predictions tagged `..._ts_on.json`. Pair with `fact_time_anchor` for real timestamps. **Status: code complete + syntax-checked + unit-tested (proximity math); NOT yet validated on a real LLM run** — best tested on timeqa (`time_specific`), since complextr questions may lack parseable years.
- **Information filtering (opt-in, default off).** `--info_filter true` drops degenerate triples (`_is_informative_triple`: subject/object that is a bare pronoun/stopword in `_LOW_INFO_TOKENS`, empty, or <2 chars) **before** graph construction, in `index()` right after `chunk_triples` is built (so entity nodes, facts, and D1 all see the filtered set). Adapted from DyG-RAG's "explicit subject, no pronouns" rule — conservative, no LLM. Changes the graph → separate working dir (`main.py` appends `_if`, composes with `_ft`). The `main.py` OpenIE-reuse convenience now covers any derived dir (`_ft` and/or `_if`): it strips those suffixes to find the base (plain) OpenIE cache and copies it in, so the derived graph is built from the same triples as the baseline. **Status: code complete + syntax-checked + unit-tested; not yet validated — benefit uncertain (prunes edges → could help noise or hurt recall), measure on timeqa.**
- **A — temporal-proximity edges (opt-in, default off).** `--temporal_edges true` runs `add_temporal_edges` after `add_fact_edges` in `index()`: for each hub entity (appears in many facts), it connects the *other* endpoints of pairs of those facts when the facts are close in time (`|Δyears| ≤ temporal_edge_window`, default 2.0), adding weight `temporal_edge_weight * exp(-temporal_edge_alpha*|Δyears|)` into `node_to_node_stats` (on top of the count-based fact edges). This builds **temporal bridges so PPR can chain facts within the same period** — adapted from DyG-RAG's event graph, targeting HippoRAG's time-blind multi-hop weakness (the −12pp gap to DyG on ComplexTR). Per-hub list capped at 50 to bound O(m²). Requires real per-fact timestamps → **pair with `--fact_time_anchor`** (no-op without). Changes the graph → separate `_te` working dir (composes `_ft`/`_if`; `main.py` OpenIE-reuse strips all of `_te`/`_if`/`_ft` to find the base cache). Knobs: `--temporal_edge_window`/`--temporal_edge_alpha`/`--temporal_edge_weight`. **Status: code complete + syntax-checked + unit-tested; NOT yet validated.** This is the most-promising un-adopted DyG idea that fits the PPR substrate (vs event-units which would mean rebuilding into DyG).
