...
- **D2 — timeline + Time-CoT (opt-in, default off).** `--time_cot true` makes `retrieve()` attach the reranked `top_k_facts` with their edge timestamps (`_fact_edge_time`, looked up from the graph so it works in load-only sessions too) onto `QuerySolution.facts`; `qa()` then renders them chronologically via `_build_timeline_block` ("Event N [time]: s p o", timeless facts as Background) plus a temporal-reasoning instruction (adapted from DyG-RAG `dynamic_QA`/Time-CoT), prepended to the QA prompt before the passages. **QA-time only — does NOT touch the graph, so it reuses the working dir** (no new suffix); predictions are saved as `predictions_tw_{on,off}_cot_on.json` and `eval_dyg.py` matches them. Inert unless edges carry real timestamps → pair with `--fact_time_anchor` on real datasets. **Status: code complete + syntax-checked; NOT yet validated on a real LLM run.** Conceptual limit: D2 only helps the LLM reason over already-retrieved evidence — it does NOT make *retrieval* time-aware (wrong-time passages stay wrong); that is D3 (time-scoped retrieval), still unbuilt.
